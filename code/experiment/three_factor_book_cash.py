#!/usr/bin/env python3
"""pos64 / RSI / -PVR rotation + BOOK lots + cash>20%. Seek stable yearly Sharpe, low DD."""
...
def gate_trail(pnl: pd.Series, trail: pd.Series) -> pd.Series:
    t = trail.reindex(pnl.index)
    out = pnl.copy()
    out[t.fillna(0) <= 0] = 0.0
    return out
...
def main() -> None:
    ...
    for dt, day in panel.groupby("date"):
        ...
        for col, sgn, name in FACTORS:
            ...
            n2[name].append((dt, apply_hold(day, pick_ls_n(day, sc, 1))))
    ...
    ew6 = pd.concat(s6, axis=1).mean(axis=1)
    ew2 = pd.concat(s2, axis=1).mean(axis=1)
    ew_b = book_ratio(ew6)

    # trail of chosen 6-hand (shifted already inside rotate via trail_sum on components)
    trail20 = pd.DataFrame({k: trail_sum(s6[k], 20) for k in s6})
    win_trail = pd.Series({dt: trail20.loc[dt, pick20.loc[dt]] if dt in trail20.index and pd.notna(pick20.loc[dt]) else np.nan for dt in idx})
    ...
    # cash-only BOOK (6 or 0) — simple on/off
    for label, a, b, br in [
        ...
        for t0 in (1.05, 1.10, 1.15, 1.20):
            pnl, stt = sized(a, b, br, t2=t0, t0=t0, how="scale")  # no mid band
    ...
    # winner must have positive 20d trail, else cash; then BOOK on/off
    gated = r20_6.copy()
    ...
    for t0 in (1.10, 1.20, 9.0):
        pnl, stt = sized(gated, g2, gb, t2=t0, t0=t0, how="scale")
        pnl = pnl.copy()
        pnl[cash_gate] = 0.0
        if stt is not None:
            stt = stt.copy()
    ...
    # 2% flatten 5d on rotated 6-hand then BOOK cash
    ...
    rows = []
    print(f"{'scheme':48} cash  sh   dd    neg  worst  std   2026", flush=True)
    for name, (pnl, stt) in schemes.items():
        ...
        print(
            f"{name:48} {st['cash']:.0%} {st['sharpe']:.2f} {st['max_dd']:.1%} "
    ...
    ok.sort(key=lambda r: (r["n_neg_years"], -(r["worst_year_sharpe"] or -9), -(r["max_dd"] or -9), r["sharpe_std"] or 9))
    slim = lambda r: {k: r[k] for k in (
        "scheme", "return", "sharpe", "max_dd", "n_neg_years", "neg_years",
        "worst_year_sharpe", "sharpe_std", "sharpe_2026", "cash", "pct_6", "pct_2", "yearly",
    ) if k in r}
    path = OUT_DIR / "three_factor_book_cash.json"
    path.write_text(json.dumps({
        "constraint": "cash>20%; BOOK lots; rotate among pos64, rsi, -pvr",
    ...
    for r in ok[:8]:
        print(f"  {r['scheme']:48} sh={r['sharpe']:.2f} dd={r['max_dd']:.1%} neg={r['neg_years']} cash={r['cash']:.0%} 2026={r['sharpe_2026']}", flush=True)
